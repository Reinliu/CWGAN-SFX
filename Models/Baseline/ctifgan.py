...
def generator(z_dim = 100, use_batch_norm = True, n_classes=5):
    ...
    for i in range(4):
        ...
        #x = tfa.layers.SpectralNormalization(Conv2DTranspose(filters=generator_filters[i], kernel_size=(12, 3), strides=(2, 2), padding='same', name = 'upsample_conv_{}'.format(i), activation = 'relu'))(x)
        if use_batch_norm == True:
            x = BatchNormalization()(x)

    x = Conv2DTranspose(filters = 1, kernel_size = (12, 3), strides = (2, 2), padding='same', name = f'generator_Tconv_5', activation='tanh')(x)
    #x = tfa.layers.SpectralNormalization(Conv2DTranspose(filters = 1, kernel_size = (12, 3), strides = (2, 2), padding='same', name = f'generator_Tconv_5', activation='tanh'))(x)
    # x = LeakyReLU(alpha=0.2)(x)
