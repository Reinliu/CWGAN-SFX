...
class WGANGP(keras.Model):
    ...
    def train_batch(self, x, y, batch_size):
        ...
        real_images = x[idx]
        labels = y[idx]
        #real_images = tfio.audio.Spectrogram(real_images, nfft=self.n_fft, window=self.win_length, stride=self.hop_length, iterations=50)
    ...
    def train(self, x, y, batch_size, batches, synth_frequency, save_frequency, n_classes, checkpoints_path, override_saved_model):
        ...
        for batch in range(batches):
            ...
            if batch % synth_frequency == 0 :
                ...
                for i in range (n_classes):
                    ...
                    plt.imsave(f'{checkpoints_path}/synth_audio/{batch}_batch_synth_class_{i}.png', np.squeeze(generated_signal))
                    ...
                    sf.write(f'{checkpoints_path}/synth_audio/{batch}_batch_synth_class_{i}.wav',
                                             tf.squeeze(generated_audio).numpy(), samplerate = 16000)
                print(f'Done.')
